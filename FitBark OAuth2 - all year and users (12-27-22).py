...
def getDataForUserFromCredentials(credentialRow):
    ...
    if participantType is not None and fitbarkPassword is not None:
        ...
        accesstokenJSON = APIcalls.getAccesstoken(client_id,client_secret,callback_uri,authencode,accesstoken_url).json()
        ACCESS_TOKEN = accesstokenJSON["access_token"]
        print("accesstoken here",ACCESS_TOKEN )
        ...
        dogActivityData = APIcalls.getActivityDataForDog(client_id,client_secret,ACCESS_TOKEN, dates)
        print(dogActivityData)
        csvdata = parseActivityDataAsCSV(dogActivityData,participantType,uniqueID,familyID,fitbarkEmail,studygroup)
        return [csvdata, fitbarkEmail, ACCESS_TOKEN]
...
def saveIndividualParticipantData(userCSVData,email,accesstoken):
    header = "Date,Participant Type,Unique ID,Family ID,Email,Group,Rest (minutes),Play (minutes),Active (minutes),Activity Value," + "Accesstoken," + accesstoken
    userCSVData = header + "\n" + userCSVData
    today = datetime.today()
    dateStringIndividual = today.strftime("%m-%d-%Y %I.%M.%Ss %p")
    dateString = today.strftime("%m-%d-%Y")
    newfolder = "./FitBark/"+ dateString + "/Individual_Data/"
    createNewFolderIfNeeded(newfolder)
    with open(newfolder + email + " " + dateStringIndividual + ".csv","w") as w:
        w.write(userCSVData)

#-------------------------------------------- code starts here ---------------------------------------------------------------------------------------
filepath = "Study Device Log-Ins Sab 12-27-2022.xlsx"
...
for row in userDataSheet.iter_rows():
    # if index > 1:
    #     break
    userCSVDataArray = getDataForUserFromCredentials(row)

    if userCSVDataArray is not None:
        masterCSVString += userCSVDataArray[0]
        saveIndividualParticipantData(userCSVDataArray[0], userCSVDataArray[1], userCSVDataArray[2])

    index += 1

header = "Date,Participant Type,Unique ID,Family ID,Email,Group,Rest (minutes),Play (minutes),Active (minutes),Activity Value"
